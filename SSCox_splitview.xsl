...
the body tags of the Web page. -->
         ...
         <body
            ...
            <p align="center">
               <a href="http://people.cohums.ohio-state.edu/ulman1/SSCoxJournal/" target="self"
                  ...
                  target="self"> Reading</a> | <a
                  href="SSCoxJournal-Diplomatic.html"
                  target="self"> Diplomatic</a> | MS Image/Text (current/<a
                  href="#view"
                  >about</a>)
               <br/>
            </p>
            ...
            <xsl:for-each select="//app">
               <p class="hang"><a>
                  ...
                  <xsl:number count="//app" format="a" level="any"/>.&#xA0;<xsl:value-of
                     select="lem"/>&#xA0;]&#xA0; <xsl:value-of select="rdg"/>&#xA0;<i>
            ...
            <p><a name="view"/>This view incorporates images of the journal and manuscript pages
               (the images link to scalable page images); in all other respects it is identical to
               the diplomatic view. The text of the journal is organized by manuscript pages. Line
               breaks and page breaks reflect those in the manuscript; paragraph breaks are not
               reported. Spelling, punctuation, capitalization, and abbreviations are reported as
               they appear in the manuscript. Text highlighted by Cox with an underscore is
               underscored in this view. Recoverable text canceled by Cox is reported in red,
               strikethrough text; unrecoverable text canceled by Cox is reported as three x's
               (regardless of the number of characters canceled) in red, strikethrough text. Text
               added by Cox between lines is reported between arrows (&#x2191; &#x2193;)
               indicating the position of the addition; text added in the margin is reported between
               pipes (|). All material added by the editor is surrounded by square brackets:
               uncertain readings are enclosed in square brackets and followed by a question mark,
               errors in the manuscript are indicated by <em>sic</em>, and text supplied by the
               editors for clarity is set in italics. Gaps in the manuscript (e.g., tears) are noted
               by ellipses within square brackets. Links to explanatory notes are numbered, colored,
               and underlined; links to textual annotations are lettered, colored, and underlined.
               Finally, the background consists of a tiled detail from a page of Cox's journal.</p>
            <hr/>
            <p align="left">
               <a href="http://people.cohums.ohio-state.edu/ulman1/SSCoxJournal/" target="self"
                  >Home</a>
   ...
   <xsl:template match="pb">
      ...
      <xsl:choose>
         <xsl:when test="@audiostart">
            <br/>
            <span class="pagebreak"> [Page&#xA0;-&#xA0;<xsl:number count="pb" format="1"
                  level="any"/>&#xA0; (<a><xsl:attribute name="HREF"
                  ...
                  page image and audio file in a new window</a>)]<br/>
               <a>
                  <xsl:attribute name="HREF"
                        >http://people.cohums.ohio-state.edu/ulman1/SSCoxJournal/CoxZoom.cfm?file=<xsl:value-of
                        select="@mmid"/>.jpg</xsl:attribute>
                  <xsl:attribute name="target">top</xsl:attribute>
                  <img>
                     <xsl:attribute name="src"
                           >https://images.asc.ohio-state.edu/is/image/mediamanager<xsl:value-of
                           select="@mmid"/>.jpg?scale=0.2&amp;fmt=png</xsl:attribute>
                  </img>
               </a>
            </span>
         </xsl:when>
         <xsl:otherwise>
            ...
            <span class="pagebreak"> [Page&#xA0;-&#xA0;<xsl:number count="pb" format="1"
                  level="any"/>&#xA0; (<a><xsl:attribute name="HREF"
                     ...
                     >top</xsl:attribute>click to open page image in a new window</a>)]<br/>
               <a>
                  <xsl:attribute name="HREF"
                        >http://people.cohums.ohio-state.edu/ulman1/SSCoxJournal/CoxZoom.cfm?file=<xsl:value-of
                        select="@mmid"/>.jpg</xsl:attribute>
   ...
   <xsl:template match="lb">
      ...
      <a>
         <xsl:attribute name="name">
            <xsl:number count="lb" format="001" level="any"/>
         </xsl:attribute>
      </a>
   ...
   <xsl:template match="div1/opener/dateline">
      <xsl:choose>
         <xsl:when test="ancestor::div1[@audiostart]"> [<a><xsl:attribute name="href"
                  ...
                  open)</xsl:attribute><xsl:attribute name="target"
               >top</xsl:attribute>&#187;</a>]<strong>
               <xsl:apply-templates/>
            </strong>
         </xsl:when>
         <xsl:otherwise>
